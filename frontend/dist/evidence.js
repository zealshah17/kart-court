...
export function evidenceCards({ product, dimensions, conversation, name, width, available, photos = [], notes = [] }) {
  ...
  if (product) {
    ...
    cards.push({ title: 'Customer reviews', label: count ? 'Review sample' : 'Missing', body: count ? `${count} readable reviews included in the conversation` : 'Review text unavailable', detail: [product.rating != null ? `Listing rating: ${product.rating}${product.ratingCount != null ? ` (${product.ratingCount} ratings)` : ''}` : 'No listing rating supplied.', count ? 'The supplied sample may not represent all buyers.' : 'A star rating is not review text. Add reviews to the product JSON to include customer experiences.'].join('\n') });
  }
  ...
  for (const [speaker, label] of [['angel', 'Angel’s case'], ['devil', 'Devil’s case']]) {
    ...
    if (lines.length) cards.push({ title: label, label: speaker === 'angel' ? 'Pros' : 'Cons & questions', body: lines[0].text, detail: lines.map(line => line.text).join('\n\n') });
  }
  for (const question of conversation?.missingEvidence || []) cards.push({ title: question, label: 'To check', body: 'Open question from this case', detail: question, action: 'note' });
