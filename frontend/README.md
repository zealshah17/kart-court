...
At the end of a hearing, the existing verdict dialog offers Yes / No. Selecting either closes the dialog and reveals a generated pixel judge at the center of the scene, with a three-strike CSS gavel animation and a humorous verdict. Browser speech reads it when supported; the text always remains visible. Reduced motion disables the animation. Yes expresses purchase intent; it does not place an order. Evidence detail dialogs retain Back to court without purchase choices.

`dist/verdict.js` owns the decision copy and presentation lifecycle. `dist/app.js` connects it to hearing completion. `dist/styles.css` handles the sprite frames and responsive layout. The generated sheet and prompt are in `dist/assets/judge-gavel.png` and `judge-gavel.prompt.txt`. Start a new hearing or return to court to dismiss the judge.
